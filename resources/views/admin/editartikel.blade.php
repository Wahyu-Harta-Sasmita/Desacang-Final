<x-sidebar-layout>
    ...
    <div class="bg-white p-8 rounded shadow w-full mx-auto mt-10">
        ...
        <form action="{{ route('artikel.update', $artikel->id_article) }}" method="POST" enctype="multipart/form-data"
            class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    @if($artikel->cover_article)
                        <p class="text-sm text-gray-500 mt-2">Cover saat ini:
                            <img src="{{ asset('assets/uploads/artikel_cover/' . $artikel->cover_article) }}" alt="Cover"
            ...
            <div class="text-center space-x-4">
                ...
                <button type="submit" class="bg-blue-500 text-white px-6 py-3 rounded hover:bg-blue-600">Simpan
                    Perubahan</button>
            </div>
        </form>
    </div>
